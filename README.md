...
- `results/` — **Outputs generated by the code**
  - `results/figures/` — figures from imaging & BRB-seq
  - `results/tables/` — CSV/TSV tables (counts, DE results, summaries)

- `docs/` — Notes & method references (optional screenshots/schematics)

- Root files:
  - `environment.yml` — Python environment for imaging
...
| Task | Where to look |
|------|----------------|
| 3D imaging segmentation / quantification (Python) | `scripts/python/` |
| Imaging plotting or small R helpers | `scripts/R/` |
| BRB-seq full pipeline (QC → DE → enrichment) | `notebooks/BRBseq_analysis.Rmd` |
